...
test('timeline basics, snapping, loop handles, asset drop, export', async ({ page }) => {
  ...
  const imageRowTestId = await imageRow.getAttribute('data-testid')
  expect(imageRowTestId).toBeTruthy()
  const imageAssetId = imageRowTestId?.replace('asset-row-', '')
  expect(imageAssetId).toBeTruthy()
  const imageClipTestId = `clip-${imageAssetId}-clip`
  await imageRow.getByRole('button', { name: /Send to V2/i }).click()
  await page.locator('.tabs .tab', { hasText: 'Edit' }).click()
  await waitForRenderIdle(page)
  await expect(page.getByTestId(imageClipTestId)).toBeVisible()
  await page.locator('.tabs .tab', { hasText: 'Assets' }).click()
  await waitForRenderIdle(page)
  await imageRow.click({ button: 'right' })
  const assetMenu = page.getByTestId('asset-context-menu')
  await expect(assetMenu).toBeVisible()
  await assetMenu.screenshot({ path: 'screenshots/asset-context.png' })
  const unassignButton = page.getByTestId('asset-unassign')
  await expect(unassignButton).toBeEnabled()
  await unassignButton.click()
  await expect(assetMenu).toBeHidden()
  await page.locator('.tabs .tab', { hasText: 'Edit' }).click()
  await waitForRenderIdle(page)
  await expect(page.getByTestId(imageClipTestId)).toHaveCount(0)
