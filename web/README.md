...
| Edit overview | Assets tab | Asset context | Beat detection | Text font sync | Export tab |
| --- | --- | --- | --- | --- | --- |
| ![Edit overview](./screenshots/edit-overview.png) | ![Assets](./screenshots/assets.png) | ![Asset context](./screenshots/asset-context.png) | ![Beat detection](./screenshots/beat-detect.png) | ![Text font sync](./screenshots/timeline.png) | ![Export](./screenshots/export.png) |
...
- Preview: program monitor synced to playhead with safe guides and overlay support.
...
- Text · Font Sync: one line of text cycles through multiple fonts on each beat/marker with size/opacity/position controls.
- Text · Beat Sync: multiple lines mapped sequentially to beats/markers for lyric-style overlays.
- Export: mock presets (JSON/MP4/WebM) plus Playwright smoke that records screenshots + a run video.

## Features exercised in the UI
- Tracks: draggable clips, trim handles (ripple/roll/slip/slide), marquee multi-select, undo/redo.
- Snapping: grid/marker/gap/edge targets with labeled snap ghost.
- Looping: minimap handles + overlay; playhead loop.
- Navigation: wheel+modifier zoom, Shift+wheel pan, zoom-to-selection, ruler click.
- Assets: metadata, waveform/thumb, drag/drop or “Send to track”.
- Markers: list jumps playhead; add markers at playhead; import from beats.
- Text on beats: Font Sync + Beat Sync overlays driven by detected beats/markers.
- Export: preset mock (JSON/MP4/WebM) and downloadable payload.
...
- E2E smoke (records video + screenshot): `npm run test:e2e`
  - Outputs: `screenshots/edit-overview.png`, `screenshots/assets.png`, `screenshots/beat-detect.png`, `screenshots/export.png`, `screenshots/timeline.png`, and `screenshots/timeline-run.webm`.
